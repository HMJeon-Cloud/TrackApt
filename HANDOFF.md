...
> 현재 버전: **v5.49** (수집기 v2.14) · 배포: https://track-apt.vercel.app · 저장소: `TrackApt`
...
**다음에 할 일 (순서대로)**
1. ~~`api/apt.js` 수정~~ → **확인 완료(2026.09): 이미 `addr: kaptAddr`·`roadAddr`·`useDate`를 넘긴다.**
   수집기 v2.14가 그대로 읽어 `j`(번지)·`ud`(사용승인연도)를 싣는다. 프록시는 손댈 것 없음.
