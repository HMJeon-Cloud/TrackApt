# TrackApt 인수인계 (HANDOFF)

> 새 세션에서 이 파일 + `index.html` + `kb_data.json` + `PROJECT.md`를 올리면 그대로 이어서 작업할 수 있습니다.
> 현재 버전: **v5.48** (수집기 v2.13) · 배포: https://track-apt.vercel.app · 저장소: `TrackApt`

---

## 1. 한 줄 요약

전국 아파트를 **가격 서열(급지) → 내 자금이 닿는 구간 → 그 구간 최상단 지역 → 조건 통과 단지** 순으로 좁혀 내려가는(탑다운) 부동산 분석 웹앱. 단일 `index.html`(ES5, 약 6,100줄) + Vercel 서버리스 프록시 4개. 브라우저 전용 워크플로우(GitHub 웹 UI 업로드 → Vercel 자동 배포).

---

## 2. 작업 방식 (반드시 지킬 것)

- **매 수정은 python `str.replace` 패치 + `assert count == expect`**로 진행. 앵커가 안 맞으면 패치가 조용히 실패하므로 건수 확인 필수.
- 수정 후 **`node --check`**(문법) → **jsdom 통합 테스트**(Chart/canvas/kakao/fetch 목업)로 검증한 뒤 `/mnt/user-data/outputs/`에 복사하고 `present_files`.
- 버전 배지 `<span class="ver">vX.Y</span>`를 매번 올림.
- 사용자(HM)는 초·중급 개발자, 한국어로 소통, **번호 매긴 단계별 + 체크포인트**를 선호. 로컬 개발환경 없음 — 전부 브라우저.
- 응답 마지막에 항상 **체크포인트**(배포 후 확인할 항목)를 제시.

## 3. 파일·환경

```
/home/claude/trackapt/
  index.html          # 본체 (ES5 단일 파일, <script> 인라인)
  collect.html        # 실거래 수집기 v2.6.1 (코드 탐지·셀 보충·분할 저장·파일 점검·market_core 생성)
  kb_data.json        # KB 시계열 (0.95MB, 정적 — 사용자가 매달 교체)
  supply_data.sample.json  # 확장 수급 데이터 견본 (선택 파일 스키마)
  PROJECT.md          # 버전별 변경 누적 로그
  api/{trades,apt,rent,supply}.js   # Vercel 서버리스 프록시
```

- 출력은 `/mnt/user-data/outputs/`에 복사해야 사용자가 받을 수 있음.
- 검증 루틴 예시는 PROJECT.md와 과거 세션 참고. jsdom에서 `HTMLCanvasElement.prototype.getContext`를 목업하고 `Chart.defaults={}` 세팅 필요.

## 4. 키 관리 (확정)

- **카카오 JS 키**: `2d591b6ef9c70f94f369b7c55c91baa3` — `index.html` 상단 `var KAKAO_JS_KEY` 상수에 **항상 하드코딩**. `kakaoKey()`가 상수 > localStorage 순. JS 키는 도메인 제한으로 보호되므로 소스 포함 안전. 카카오 콘솔 플랫폼(Web)에 `track-apt.vercel.app` 등록 필요.
- **공공데이터 인증키** `MOLIT_API_KEY`: **절대 index.html에 넣지 않음.** Vercel 환경변수로 관리, `api/` 프록시가 서버측에서 사용(trades.js:25, apt.js:124, rent.js:18, supply.js:14).

## 5. 데이터 출처 구조 (중요)

| 화면·값 | 출처 |
|---|---|
| 매매·전세가·갭·거래건수·평당가·준공연도 | **국토부 실거래** `/api/trades` |
| 지역 시세·지수·상승률·분위·밴드·거래량 | **market_core.json** (실거래 집계본, 정적) |
| 세대수·주차·지하철·학군·편의·시공사 | **K-apt** `/api/apt` (의무관리대상만 등록) |
| ~~KB `kb_data.json`~~ | **v5.0에서 제거** (저작권 부담 · market_core로 대체) |
| 입주 예정(분양공고) | **청약홈** `/api/supply` (실시간) |
| 미분양·준공실적·인구 | **supply_data.json** (선택 파일, 없으면 해당 분석 off) |

- **KB는 정적 파일이라 자동 갱신 안 됨** — 매달 KB 데이터허브 엑셀 → `kb_data.json` 재생성 → 교체.
- 실거래·K-apt·청약홈은 조회마다 실시간(+캐시).

## 6. 핵심 상수·구조 (코드에서 자주 건드리는 것)

- `KAKAO_JS_KEY` (상단 상수)
- `RULES` (기준·설정, 약 206줄): `wRank:35, wPerf:25, wSupply:15, wTiming:15, wCash:10` · LTV/DSR/취득세/총액상한
- 필요현금 = 매수가 − min(LTV, 총액상한, DSR) + 취득세 + 중개 0.4% + 부대. 상한: 15억↓ 6억 / 15~25억 4억 / 25억↑ 2억
- `CART_W = {rank25, perf20, timing15, supply10, danji15, cash15}` (장바구니 6축)
- `GROUPS` (약 250줄): 2단 내비 6그룹
  - 시작하기: `start`(내 조건) · `reco`(추천) · **`faq`** · `guide`(사용법)
  - 시장 공부: `market` · `vol`(거래량) · `quad`(매매·전세 흐름) · `supply`(입주물량) · `btest`(과거 검증)
  - 어디를 살까: `rank`(지역 1등) · `mapx`(지도) · `gap`(전세끼고) · `move`(갈아타기) · `rota`(다음 오를 곳)
  - 어떤 아파트: `flag`(대장) · `danji`(아파트 찾기) · `cmpx`(비교)
  - 돈·설정: `calc` · `setting`(진단 도구 포함)
  - 내 후보: `cart`
- `RANKED`: salePm 표본 24개월 미만 지역 제외, 정상 하위 구 없으면 상위 시 유지
- 데이터 접근: `getVal(key,region,y,m)`, `lastVal`, `latestYM`, `ymNum`, `disp`, `ik()`, `kbSido`, `kbLabel`

## 7. 화성 분구 처리 (v4.4에서 종결 — 개별 코드 직접 조회)

**v4.4부터 화성 4구는 각자 코드(41591 만세 · 41593 효행 · 41595 병점 · 41597 동탄)로 직접 조회한다.**
실거래 API가 과거 거래도 현재 구 코드로 돌려주기 때문이다 (2010.01~2026.06 전수 수집으로 확인:
41590은 198개월 내내 0건). `fetchHwa`·`SUB_DISTRICT` 동 필터는 삭제됐고, `HWA`는 K-apt 병합용으로만 남았다.
같은 이유로 부천 3구·인천 신설 4구·전남광주통합 12xxx도 전부 개별 코드에 전 구간 데이터가 있다.
아래는 v3.5 시점 기록(참고용, 더 이상 코드에 없음).

### (구) v3.5 기록

- 확정 코드(행안부 주민등록 행정구역 코드 기준):
  **41591 만세구 · 41593 효행구 · 41595 병점구 · 41597 동탄구**
  (41592 동부출장소 · 41594 동탄출장소는 구가 아님 → 제외, 41590은 옛 화성시)
- `HWA = {old:"41590", news:[4개], guOf:{코드→"화성 ○○구"}, SPLIT_YM:202602}` — 정적.
  `probeHwaCodes`·`guessHwaGu`·`HWA_ANCHOR`는 삭제됨(탐지 불필요).
- `fetchHwa(kind, ym, guName)`: 202602 미만은 41590만, 이후는 guName이 있으면 그 구
  코드 1개, 없으면 4개 병합(`mergeTradeResults` dedupe). 병합 결과 0건이면 41590으로
  1회 재조회(이관 지연 대비).
- `LAWD_GROUPS`의 화성 4구는 여전히 **41590으로 매핑**되어 있다(의도적).
  `fetchTrade`가 41590을 보고 `fetchHwa` 경로로 보내야 분구 전 과거 시계열이
  이어지기 때문. 개별 구 코드로 바꾸면 2026.2 이전 데이터가 사라진다.
- 설정 탭의 "지역 코드 진단"(`runDiag`)은 목적 달성 후 v3.5에서 제거됨.

## 8. 성능·안정화 (v2.x~v3.x)

- `apiGet`: 동시 3개 큐 제한 + 502/429/요청제한 시 백오프 2회 재시도. **모든 공공 API 호출이 이 경로.**
- 캐시: `TRADE_MEMO`(kind|lawd|ym) · `KAPT_INFO_MEMO` · `KAPT_LIST_MEMO` · `SUPPLY_MEMO`(시도별)
- 아파트 찾기: 실거래 도착 즉시 렌더 → K-apt는 백그라운드 병렬 부착(0.5초 배치, `DJ.loadSeq`로 이전 조회 무시)
- K-apt 이름 매칭: `normName`+`nameScore`(문자겹침+접두+동 가산, 0.62 임계) — "향촌롯데↔향촌마을롯데" 등 흡수

## 9. 최근 작업 (v3.1~v3.4) — 새 세션에서 이어질 맥락

- **v3.1**: FAQ 탭 신설(Q1~Q17 백테스트, 아코디언). 입주물량 탭 자동 조회.
- **v3.2**: 입주물량 **전국 집계 기본** — 17개 시도 병렬 조회 병합, KPI 5종, "수급 읽기" 자동 요약(배율 상·하위, 물량 top3), 시도 접두 표.
- **v3.3**: FAQ **라이브 재계산** — Q1·Q2·Q12·Q15는 `faqCompute()`가 내장 KB로 재계산해 `fb-q*`에 주입. 나머지 13개는 "고정 검증" 배지.
- **v3.4**: 
  - FAQ 주기 UI 제거 → kb_data.json 교체 감지 시 자동 재계산 + "지금 다시 계산" 버튼만.
  - **`supply_data.json`(선택 파일) 신설**: 있으면 입주물량 탭에 미분양 추이 카드 + 공급·수요·매매가격 연도별 카드 + 시군구 표 확장(인구·수요·배율·준공후미분양). 없으면 청약홈만으로 동작 + 안내.
  - 사용법 탭에 "쓴 자료 전체" 출처 표(`guideSourceTable`).
- **v3.5**:
  - 화성 코드 확정(위 7번) · 진단 도구 제거.
  - **데이터 관리 탭 신설**(`data`, 돈·설정 그룹): kb_data.json·supply_data.json을
    브라우저에서 업로드 → IndexedDB(`trackapt`/`files`) 보관. 저장본 우선, 없으면
    저장소 정적 파일. 형식 검사 실패 시 저장 안 함. `DATA_SRC={kb,supply}`로 출처 표시.
  - **supply_data.json 실데이터 장착**(미분양 198개월·256지역 / 준공 2011~2025 /
    입주예정 시도별 2026~2027 / 인구 2026.06). 생성기 `make_supply.py` 동봉.
- **v3.6**:
  - **지역 선택기**: optgroup 셀렉트 → 버튼+아코디언 모달 자동 승격
    (`upgradeRegionSelects`/`openRegionPicker`). 원본 select 유지, `selectedIndex`로
    선택(화성 4구는 value가 같아 value로는 구분 불가). `refreshHomeMarks`가 래퍼.
  - **규제지역 2026.7.1 기준**: 서울 25구 + 경기 15곳(화성 동탄구·용인 기흥구·구리시
    추가). `RULES.regVer`로 저장본 자동 마이그레이션.
  - **fetchTimeout(20초)**: 응답 지연 시 apiGet 큐가 막혀 전체 조회가 멈추던 문제 해결.
  - 사용법 출처 표를 탭 진입마다 재렌더(`refreshSourceTable`) — 항상 "미장착"으로
    보이던 버그 수정.
- **v3.7**:
  - **CSV 자동 변환**(`buildSupplyFromCsv`·`csvKind`·`decodeKr`·`ymPick`): 통계누리
    미분양/준공, 행안부 인구 CSV를 브라우저에서 직접 supply_data로 변환·저장.
    make_supply.py와 결과 일치(테스트 검증). 올린 종류만 교체, expected는 보존.
    잠정치 표기("2025-10(잠정 p)") 흡수가 핵심 — 없으면 최근 연도가 통째로 빠진다.
  - 카트 주변환경 재조회에 `envTried` 플래그 — K-apt 항목이 원래 비어 있는 경우를
    "다시 눌러보세요"와 구분해 표시.
  - 추적 방지(엣지·사파리)로 IndexedDB 저장이 막힐 수 있음을 데이터 탭에 안내.
- **v4.3**: 자산 레이스 — 지수·주식 연말 종가 상수 6종 추가(NVDA 2025만 근사,
  나머지 확정). 대표 단지(압구정 현대·부산 삼익비치) 온디맨드 실거래 로더
  `loadRepComplexes`(~58회, 캐시). **매년 갱신 상수 총 10종** (WAGE·FX·GOLD·BTC·
  KOSPI·KOSDAQ·SPX·NASDAQ·SAMSUNG·NVDA).
- **v4.2**: 자산 대결 카드 — `FUN_GOLD`/`FUN_BTC` 연말 종가 상수(2025는 검색 검증:
  금 $4,320 · BTC $87,500). 로그 축 필수. 매년 상수 4종에 전년치 추가.
- **v4.1**: "재미로 보기" 탭(`fun`) — 정권별 지수(10개 정권, 연복리) · 정책 연표
  17건(+6/12개월 변화) · 최저시급/환율 환산(상수 `FUN_WAGE`/`FUN_FX`, 매년 갱신 필요:
  시급 확정치·환율 연평균 추가) · 강남÷전국 배율. buildFun은 셸만, 셀렉트는
  fillFunSelects()가 첫 렌더에 채움.
- **v4.0**: 준공후 미분양(`unsoldDone`) 장착 — 통계누리 "공사완료후 미분양현황"은
  6열(부문·규모) 스키마라 부문=계·규모=계만 취하고 전국 행이 이미 있어 합산 금지.
  `make_supply.py build_unsold(pattern)`이 두 형식 공용. 시군구 표에 미분양·준공후 2열.
- **v3.9**:
  - **전앱 % 버그 수정**: v3.3의 FAQ용 전역 `pct(x·분수)`가 호이스팅으로 본래
    `pct(v·퍼센트)`를 덮어써 모든 % 표시가 ×100이던 문제 → `fpct`로 개명.
    ("+549.4%/년" 같은 값이 보이면 이 버그 재발 신호. 전역에 pct 재선언 금지.)
  - **과거 검증 4모드**(`BT_MODES`·`groupComplexesBt`): 평당가/세대수/평형/노후도,
    기간 5·10·15년. size 모드만 K-apt 호출(상위 40단지 상한), age는 실거래
    buildYear로 무호출.
  - **전국 등급표 탭**(`tiers`·`renderTiers`): 10분위, KB만으로 계산, 시·도 필터는
    표시만 거르고 서열·분위는 전국 고정. 전체 CAGR은 `fullCagr`(기산점 상이 주의).
- **v3.8**: **데이터 업로드 탭·IndexedDB·CSV 변환기 전부 제거**. kb_data는 다중 시트
  엑셀이라 브라우저 변환이 불가능해 경로를 하나로 통일했다. 갱신은 항상
  `원본 → 세션에서 변환 → GitHub 교체 → Vercel 배포`. `loadData`/`loadSupplyExt`는
  정적 파일 fetch로 원복. (v3.5~v3.7에서 넣었던 업로드 관련 코드는 되살리지 말 것.)

## 10. supply_data.json 스키마 (v3.4)

```jsonc
{
  "updated": "2026-08",
  "unsold":     { "dates": ["2015-07","2026-06",132], "series": { "전국": {"o":0,"v":[...]}, "경기": {...} } },
  "unsoldDone": { "dates": ["2015-07","2026-06",132], "series": { "전국": {...}, "경기 평택시": {...} } },
  "completions":{ "years": [2010,...,2025], "sido": { "전국":[...], "경기":[...] } },
  "expected":   { "items": [ {"sido":"경기","sigungu":"평택시","ym":202611,"households":9000} ] },
  "population": { "total":51088284, "sido": {"경기":13650000}, "sigungu": {"경기 평택시":620754} }
}
```
- 지역 키: `"전국"` / 시도 축약(`"경기"`) / 시군구 `"경기 평택시"`.
- v3.5부터 **실데이터**가 들어있다. 재생성은 `make_supply.py`(원자료 CSV 4종 필요).
- `unsoldDone`(준공후 미분양) v4.0부터 장착 (257지역×198개월).

## 11. 다음 할 일 후보 (PENDING / BACKLOG)

> **먼저 읽을 것** — 아래 취소선 항목은 **이미 끝난 일**이다. PROJECT.md는 최신이 위라
> 파일 끝의 옛 진단("…재수집이 필요하다" 등)을 남은 할 일로 오독하기 쉽다(2026.08 실제 사례).
> 데이터 상태는 서술이 아니라 `market_core.json`의 `stat`으로 확인한다.

1. ~~화성 코드 고정~~ · ~~supply_data 실데이터화~~ · ~~준공후 미분양~~ ·
   ~~**전월세 전 구간 재수집**~~ · ~~market_core.json~~ · ~~KB 제거~~ → 전부 완료 (v5.2)
   · 전월세 재수집 근거: `stat.rentRows` 13,129,197 · **`stat.rentBlank` 0** (배포본 확인 완료)
2. ~~**3층 구조 2층** — `trades/{lawd}.json`(단지×월×면적대) 지연 로드~~ → 완료.
   `loadStore()`가 쓰고 추천받기·비교·채점표가 저장본으로 돈다(배포 확인: trades/41135.json, cx 96).
   실거래 API는 결과 화면의 최근 6개월 시세표에만 쓴다.
3. ~~저장소에서 `kb_data.json` 삭제~~ → index.html의 참조 0건(제거 완료).
   저장소에 파일이 남아 있다면 지워도 앱에 영향 없다.
4. FAQ "고정 검증" 13문항 본문의 KB 서술 정리 (검증 숫자는 2026.07 고정치 유지)
5. **K-apt 이름 불일치 감시** — v5.42가 세대수 미확인 신축의 성과를 40점으로 누르므로,
   표기가 달라 매칭에 실패한 **대단지**가 억울하게 눌릴 수 있다. 사용 중 그런 단지가
   보고되면 `matchKaptAll`의 별칭 규칙을 보강한다.
6. 잠재: Capacitor 패키징 (App ID `com.hmjeon.trackapt`)

## 12. 테마 (v2.4 확정 — 다크 럭셔리)

`--bg #0b1017` · `--panel #131a24` · 골드 `--brand #d4af6a`(`--gold-grad`) · 상승 `--up #2fbf9b` · 하락 `--down #e08c3a` · 현금 `--cash #e8b04b`. 활성 버튼·CTA는 골드 그라데이션+어두운 글씨, 로고 accent 그라데이션 텍스트, 카드 inset 하이라이트+딥 섀도. Chart.js 전역색·레이더 그리드·지도 핀(골드톤)·이미지 저장 배경까지 다크 통일. 규제지역은 "규제" 앰버 배지, 내 지역은 표·차트에 골드 강조(`tr.mine`, `regionCell`, `regionLabel`).

---

## 13. 단지 정보 연동 — 조사 결론과 남은 길 (2026.09)

**왜 빠지나** — 단지 정보(세대수·주차·지하철)는 K-apt에서 오는데, 실거래와 K-apt를 잇는 **공통
키가 없다.** 실거래는 `단지명+법정동`, K-apt는 `kaptCode`. 그래서 이름으로 맞추고, 이름은
차수·띄어쓰기·영문·개명으로 어긋난다. 게다가 K-apt는 **의무관리 단지(300세대 이상 등)만** 담아
소규모 단지는 아예 없다 — 이 두 번째가 결손의 더 큰 원인이다.

**검토한 대안**
| 방법 | 결론 |
|---|---|
| 네이버 부동산 | 공개 API 없음. 내부 API 긁기는 약관 위반·차단 위험 → **쓰지 않는다** |
| 네이버 검색 API(지역) | 세대수·준공 같은 단지 속성이 없음 → 부적합 |
| **지번(번지) 매칭** | 실거래 API가 `jibun`을 이미 준다. K-apt 기본정보의 `kaptAddr`에도 번지가 있다. 같은 동·같은 번지면 이름과 무관하게 같은 단지 → **v5.48에서 앱·수집기 양쪽 기반 마련** |
| **건축물대장 표제부(건축HUB)** | 국토부 공공 API. `시군구코드+법정동코드+번+지`로 조회하면 **모든 건물**의 세대수(hhldCnt)·사용승인일·주용도·층수·주차 → K-apt에 없는 소규모 단지까지 채울 수 있는 **유일한 공공 경로**. 지번이 키라 위 항목과 세트 |
| 카카오 로컬 API(주소→좌표) | 세대수는 못 주지만 정확한 좌표 → 위치 버튼을 검색 대신 핀으로. REST 키는 서버(Vercel 함수)에서만 |

**다음에 할 일 (순서대로)**
1. `api/apt.js`의 기본정보 응답에 **`kaptAddr`(지번 주소)·`doroJuso`를 그대로 넘기도록** 한 줄 추가
   → 수집기 v2.13 `kgRow`가 자동으로 `j`를 채운다. (이 파일은 저장소 `api/`에만 있어 세션에 없다 —
   다음 세션에 올려 주면 바로 고친다.)
2. 다음 `trades/`·`kapt/` 갱신 때 v2.13으로 다시 만들면 저장본에도 번지가 실려 추천받기까지 지번 매칭.
3. 공공데이터포털에서 **"건축HUB 건축물대장정보 서비스"** 활용신청(같은 인증키로 됨) →
   `api/bld.js` 프록시 → 아파트 찾기에서 K-apt 미매칭 단지만 지번으로 세대수·사용승인일 조회
   (일일 한도가 있어 화면에 보이는 행만, 캐시 필수).
4. (선택) 카카오 로컬 REST → `api/geo.js` → 단지 좌표 → 위치 버튼 핀.


## 실거래 데이터 파이프라인 (KB 대체 작업) — 현재 위치

### 확정된 사실 (재검증 불필요)

1. **국토부 실거래 API는 과거 거래도 "현재" 행정구역 코드로 돌려준다.**
   신고 당시 코드가 아니다. 강원 42·전북 45 계열은 전 기간 0건, 51·52에 전량 존재.
   → 옛 코드 별칭 불필요. 대신 **코드표가 최신 개편에 뒤처지면 그 지역이 통째로 사라진다.**
2. **2026.7 개편 확정 코드** (2019 실거래를 공식본과 동·번지·단지로 조인해 검증):
   · 전남광주통합특별시 = **12xxx 대역**(서울 11 다음), 25개 코드
   · 인천 신설 4구 = 28125 제물포 · 28155 영종 · 28275 서해 · 28290 검단
   · 죽은 코드 = 광주 29xxx · 전남 46xxx · 인천 28110/28140/28260
   전부 collect.html 코드표에 내장 완료(시도 라벨 "전광"). 매핑표는 `신규코드_지역명_매핑.csv`.
3. **부동산원 (월)행정구역별 주택거래현황과 건수를 대조하면 안 된다.**
   기준이 다르다(신고일 vs 계약일, 주택 전체 vs 아파트). 존재 확인용으로만.
   군 지역에서 "주택 거래는 있는데 아파트 0건"은 정상.
4. **동일 행 중복을 제거하면 안 된다.** 공식본에도 동일키 중복이 존재
   (도시형생활주택 등 같은 날 복수 호실 거래).
5. **공식 다운로드본도 결함이 있다.** 2022 파일에 2020.01~02 데이터가 섞여 있고
   2022.01·02가 없음, 2025.02·03·10 잘림. 단일 파일을 절대 기준으로 삼지 말 것.
6. **전월세 실거래는 2011.01이 시계열 시작점**(신고 의무 2011년 시행). 2010년은 원자료가 없다.

### 매매 — 완료 (재작업 불필요)

- 2010.01~2026.06 · **9,123,668행** 확정. 공식본 대조 71/79개월 오차 −1~−7건(0.005%).
  어긋난 8개월은 전부 공식 파일 쪽 결함으로 판명.
- 구성: 원본 17개(`아파트_매매_실거래가_YYYY.csv`) + 보충 14개(`..._보충_YYYY.csv`).
- **병합 방법**: 원본에서 잘림 셀 행을 **전부 삭제 후** 보충본을 붙인다(부분 병합 금지).
  잘림 셀 목록 = `매매_잘림셀_347.csv` (collect.html `SALE_CAPPED`에도 내장).
  병합본은 컨테이너에만 있었으므로 세션이 바뀌면 재생성 필요.

### 전월세 — 전 구간 재수집 필요 (다음 작업)

**[치명] 옛 프록시가 월세(monthlyRent)를 정규화하지 않아, 원본 16개 파일
12,313,005행의 월세 열이 전부 빈칸이다.** 잘림과는 별개의 두 번째 결함.

- 영향: 전세/반전세 구분 불가 → **전세가율·갭투자·4분면·추천 5축이 전부 틀어진다.**
  보증금만 보고 반전세를 전세로 오인하면 전세가율이 과대 계상된다.
- 새 프록시(api-trades.js)는 deposit·monthlyRent·contractTerm·contractType을 모두 매핑.
  보충분에서 월세 비율 21.8%(2011)→47.8%(2022)→53.2%(2026)로 시장 통계와 부합 → 신뢰 가능.
- **할 일**: collect.html에서 셀 칸을 비우고 전월세 · 2011-01~2026-06 · 전체 지역 재수집.
  약 53,000회 + 페이징, 2~3시간. 코드 탐지는 캐시되어 생략됨.
- **본 수집 전 1분 점검**: 셀 칸에 `11680-202312`만 넣고 실행 → 3,443건이 나오고
  **월세 열에 값이 채워져 있으면** 정상. 이걸 확인하고 본 수집을 걸 것.
- 기존 자료 보관: 원본 16개는 잘림+월세결손이라 폐기 예정이나, **보충 16개
  (3,017,863행, 2,202셀)는 재수집 결과 검증용 대조군으로 반드시 남길 것.**
- 참고 수치(재수집 결과와 대조용): 병합 시 13,128,868행 · 186개월(2011.01~2026.06) ·
  잘림 셀 목록은 `전월세_잘림셀_2202.csv`. 실제 값이 1,000건인 셀 4개
  (41150-202112 · 41390-202006 · 41465-202111 · 41465-202509)는 정상이므로 잘림으로 오판 말 것.

### market_core.json — 생성 경로 확정 (v2.5)

**원자료는 세션에 올리지 않는다.** collect.html v2.6의 [market_core.json 만들기]로
브라우저에서 집계해 2~4MB JSON만 받아 저장소에 올린다. 스키마·집계 규칙은 PROJECT.md v2.5/v2.6 절 참조.
검증은 그 JSON을 세션에 올려 KB `salePm`과 나란히 비교하는 것으로 한다.

- **원본 + 보충본을 함께 넣어도 된다** (v2.6). 파일명의 "보충"으로 판별해 셀 단위로 교체한다.
  병합본 CSV를 따로 만들 필요 없음. v2.5에서는 이걸 안 해서 매매 347셀이 이중 계상됐다.
- 합격 기준: `stat.saleRows + stat.saleCancel == 9,123,668` · `replCells == 347` ·
  `dropped == 347,000` · `rentBlank == 0`.

### 그다음: market_core.json (KB 대체 본작업)

3층 구조로 전환한다.
① `market_core.json` — 시군구×월 집계(건수·중위 평당가·84/59㎡별), 시작 시 로드, ~3MB
② `trades/{lawd}.json` — 단지×평형대×연, 그 지역 볼 때만 지연 로드 (스키마 v2)
②' `kapt/{lawd}.json` — 단지별 세대수·지하철·건설사·주차 (스키마 v1, 선택 파일)
③ API — 2026.07~ 최신분만

첫 단계는 매매로 시제품을 만들어 KB `salePm`과 나란히 비교하는 것.
실거래 중위가는 거래 구성에 흔들리므로 3개월 이동중위·84㎡ 고정 기준을 함께 제공한다.

### 주의

- 보충·재수집 전 **반드시 api/trades.js가 새 버전인지 확인**.
  확인법: `/api/trades?kind=sale&lawd=26350&ym=201911&numOfRows=1000&pageNo=2`
  → `totalCount:1891`과 items가 나와야 정상. 로그에 "pageNo를 무시" 경고가 뜨면 옛 버전.
- 수집기 CSV의 번지는 `="1-39"` 형식(엑셀 날짜 변환 방지) — 파싱 시 `="..."` 제거.
- **엑셀 "파일을 완전하게 로드하지 못했습니다"는 데이터 손실이 아니다.** 시트 한계 1,048,576행.
  collect.html v2.4의 `파일당 최대 행수`(기본 100만)로 나눠 저장하면 열린다. 재수집 금지.
- **대용량 CSV는 세션에 올리지 말고 collect.html의 [받은 파일 점검]으로 검증**하고,
  `점검리포트_전월세.json` + `점검_셀별건수_전월세.csv`(~1MB)만 올린다.
- 신규 코드 행의 시군구 라벨은 "신규 탐지 12330" 형태 — 매핑표로 지역명 복원.
- 대용량 파일은 ZIP으로 묶어 올리면 세션 파일 수 제한을 우회할 수 있다.

*이 문서는 v4.3 시점 스냅샷입니다. 세부 변경 이력은 PROJECT.md의 버전별 섹션을 참고하세요.*

